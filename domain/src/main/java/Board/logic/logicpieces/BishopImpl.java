...
import static Board.logic.Piece.PieceName.BISHOP;

public class BishopImpl extends AbstractPiece {

    public BishopImpl(final Environment environment, Color color) {
        super(color, BISHOP, environment);
    }
    ...
    public List<CoordinatesImmutable> getPossibleMovesInner(final PiecesOnBoard piecesOnBoard) {
        ...
        for (final var coordList : directions) {
            for (Coordinates coordinate : coordList) {
                ...
                final Color colorBishop = piecesOnBoard.getColorOfPiece(_x, _y);
                if (pieces[x][y] != null) {
                    if (!colorBishop.equals(colorOtherPiece)) {
