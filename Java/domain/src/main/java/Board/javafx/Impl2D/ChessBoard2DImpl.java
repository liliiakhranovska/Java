...
class ChessBoard2DImpl extends Group implements ChessBoard {

    final public Cell2DImpl[][] cells = new Cell2DImpl[8][8];

    public ChessBoard2DImpl() { // регистрируем supplier при создании 2D-шки
    ...
    public void insertCellsToMatrix() {
        for (int x = 0; x < 8; x++) {
            for (int y = 0; y < 8; y++) {
                if ((x + y) % 2 != 0) {
                    cells[x][y] = new Cell2DImpl(Color.WHITE);
                } else {
                    cells[x][y] = new Cell2DImpl(Color.BROWN);
                }
            }
        }
    }
